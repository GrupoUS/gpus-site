...
```bash
PHASE_INFO=$(node "/home/mauricio/gpus/.cursor/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/home/mauricio/gpus/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/home/mauricio/gpus/.cursor/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
