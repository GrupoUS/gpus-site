...
|---|---|
| Library / framework docs (Astro, Tailwind v4, React 19) | `mcp__claude_ai_Context7__resolve-library-id` → `mcp__claude_ai_Context7__query-docs` |
...
**Don't introduce MCP servers for purely local ops** (git, `bun run build`, file reads — use Bash + Read tools). **Don't add backend / DB / payments MCPs** to this project — gpus-site is presentation-only static.
...
- **Bun only** (`bun install` / `bun run` / `bunx`). Never `npm` / `yarn` / `pnpm`. Cardinal rule.
