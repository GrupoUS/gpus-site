...
# Frontend — Grupo US · Site Institucional (Tier 2 — overlay authority)

> Single source of truth for pages, components, hydration, content data, external surfaces (WhatsApp + redirects + fonts), accessibility, and performance. Absorbs former `rules/integrations.md`, `rules/content.md`, `rules/a11y.md`.
> Visual canon: `${overlay}/rules/DESIGN.md` + `gpus-theme` skill.
...
| Need | Load |
|---|---|
| Universal stability checklist + smoke tests + anti-patterns + debug triage | `${overlay}/rules/stability.md` |
