...
`.claude/hooks/{session_context,task_completed,subagent_log}.py` — Python for portability. Wired in `.claude/settings.json`. Generate `errors.jsonl` + `sessions.jsonl` automatically.
...
Generic. To copy to another project: copy this skill dir + `.claude/hooks/{session_context,task_completed,subagent_log}.py` + matching `.claude/settings.json` hook entries. Database auto-resolves to project root via `.git` lookup or `EVOLUTION_PROJECT_ROOT` env var.
