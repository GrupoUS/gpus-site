...
Generic. Database auto-resolves to project root via `.git` lookup or `EVOLUTION_PROJECT_ROOT` env var. To copy to another project: copy `.claude/skills/evolution-core/` + `.claude/hooks/{session_context,task_completed,subagent_log}.py` + matching `.claude/settings.json` hook entries.
